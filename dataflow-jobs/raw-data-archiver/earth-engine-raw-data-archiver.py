...
def run():
    ...
    subscription = f"projects/{project_id}/subscriptions/earth-engine-data-pub-sub"
    output_prefix = f"gs://{project_id}-agri-datalake/raw/earth-engine"
    ...
    with beam.Pipeline(options=options) as p:
        (
            ...
            | "Write windowed files" >> fileio.WriteToFiles(
                path=output_prefix,
                shards=1,
                file_naming=fileio.default_file_naming("remote-sensing", ".jsonl")
            )
        )
...
#python earth-engine-raw-data-archiver.py --runner=DataflowRunner --project=careful-trainer-p1 --region=asia-south1 --worker_zone=asia-south1-b --worker_machine_type=e2-small --num_workers=1 --max_num_workers=3 --temp_location=gs://careful-trainer-p1-dataflow-temp/temp --staging_location=gs://careful-trainer-p1-dataflow-temp/staging --job_name=earth-engine-data-archiver-v0 --streaming --save_main_session